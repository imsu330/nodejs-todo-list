...
router.post("/todos", async (req, res, next) => {
  ...
  try {
    ...
    // 1-5. value가 존재하지 않을 때, 클라이언트에게 에러 메시지를 전달합니다.
    if (!value) {
      return res
        .status(400)
    ...
    // findOne = 1개의 데이터만 조회 , sort = 정렬한다. -order는 내림차순으로
...
router.get("/todos", async (req, res, next) => {
  //next 는 리펙토링을 생각했을때를 위해서..미들웨어로 전달해주기 위한.
  // 1. Todo모델을 이용해, MongoDB에서 'order' 값이 가장 높은 '해야할 일'을 찾습니다.
